...
def calculate_xview2_metrics(confusion_matrix):
    """
    Calculates the official xView2 metrics from a confusion matrix.
    Classes: 0 (Bg), 1 (No Damage), 2 (Minor), 3 (Major), 4 (Destroyed)
    """
    # 1. Localization F1 (Building vs Background)
    # TP: True building, Pred building. FP: True bg, Pred building. FN: True building, Pred bg.
    ...
    # 2. Damage F1s (Calculated per damage level)
    f1_dmg_classes = []
    for i in range(1, 5):
    ...
    # Harmonic mean of the 4 damage F1 scores
    f1_dmg_classes = np.array(f1_dmg_classes)
    f1_dmg_classes_safe = np.maximum(f1_dmg_classes, 1e-8) # Prevent division by zero
    f1_dmg_harmonic = 4.0 / np.sum(1.0 / f1_dmg_classes_safe)

    # 3. Overall xView2 Score
...
def evaluate():
    ...
    dataset = xBDDataset(config.PROCESSED_DATA_DIR, is_train=False)
    # num_workers=4 leverages CPU threads to keep the GPU fed without stalling
    ...
    model.load_state_dict(torch.load(os.path.join(config.CHECKPOINT_DIR, 'best_model.pth'), map_location=device))
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            # Fast, memory-efficient confusion matrix update
