...
def train():
    ...
    dataset = xBDDataset(config.PROCESSED_DATA_DIR, is_train=True)
    ...
    scaler = GradScaler() # <-- Add this for mixed precision training
    
    start_epoch = 0
    best_loss = float('inf')
    checkpoint_path = os.path.join(config.CHECKPOINT_DIR, 'latest_checkpoint.pth')
    
    # FAIL-SAFE: Stateful Checkpoint Loading
    ...
    for epoch in range(start_epoch, config.EPOCHS):
        ...
        # FAIL-SAFE: Save state after every epoch
