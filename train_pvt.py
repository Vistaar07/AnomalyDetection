...
def train_pvt_v2_b2():
    ...
    print(f"Training pvt_v2_b2 on device: {device}")
    ...
    log_filename = f"training_log_pvt_v2_b2_{run_id}.csv"

    hp_header = (
        f"# RUN ID: pvt_v2_b2_{run_id}\n"
        f"# BACKBONE: pvt_v2_b2\n"
    ...
    checkpoint_path = os.path.join(config.CHECKPOINT_DIR, f'latest_checkpoint_pvt_v2_b2_{run_id}.pth')
    best_model_path = os.path.join(config.CHECKPOINT_DIR, f'best_model_pvt_v2_b2_{run_id}.pth')
